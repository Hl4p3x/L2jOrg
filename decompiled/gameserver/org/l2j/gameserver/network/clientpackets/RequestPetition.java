...
public final class RequestPetition extends ClientPacket
{
    ...
    public void runImpl() {
        ...
        if (!AdminData.getInstance().isGmOnline(false)) {
            ((GameClient)this.client).sendPacket(SystemMessageId.THERE_ARE_NO_GMS_CURRENTLY_VISIBLE_IN_THE_PUBLIC_LIST_AS_THEY_MAY_BE_PERFORMING_OTHER_FUNCTIONS_AT_THE_MOMENT);
            return;
        }
        if (!PetitionManager.getInstance().isPetitioningAllowed()) {
            ((GameClient)this.client).sendPacket(SystemMessageId.THE_GAME_CLIENT_ENCOUNTERED_AN_ERROR_AND_WAS_UNABLE_TO_CONNECT_TO_THE_PETITION_SERVER);
            return;
        }
        if (PetitionManager.getInstance().isPlayerPetitionPending(player)) {
            ((GameClient)this.client).sendPacket(SystemMessageId.YOU_MAY_ONLY_SUBMIT_ONE_PETITION_ACTIVE_AT_A_TIME);
            return;
        }
