...
public class MDefenseFinalizer extends AbstractDefenseFinalizer
{
    ...
    public double calc(final Creature creature, final Optional<Double> base, final Stat stat) {
        ...
        if (Objects.nonNull(inv)) {
            for (final InventorySlot slot : InventorySlot.accessories()) {
            ...
            if (GameUtils.isPet(creature) && !inv.isPaperdollSlotEmpty(InventorySlot.NECK)) {
                baseValue -= 13.0;
            }
        }
        if (creature.isRaid()) {
        ...
        final double bonus = (creature.getMEN() > 0) ? BaseStats.MEN.calcBonus(creature) : 1.0;
