...
public class NotFriend implements IAffectObjectHandler
{
    public boolean checkAffectedObject(final Creature activeChar, final Creature target) {
        ...
        return (GameUtils.isSummon((WorldObject)activeChar) && target == activeChar.getTarget()) || target.getActingPlayer().getPvpFlag() > 0 || target.getActingPlayer().getReputation() < 0;
    }
    
    public Enum<AffectObject> getAffectObjectType() {
        return (Enum<AffectObject>)AffectObject.NOT_FRIEND;
    }
}
