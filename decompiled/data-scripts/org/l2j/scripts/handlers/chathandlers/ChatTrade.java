...
public final class ChatTrade implements IChatHandler
{
    ...
    public void handleChat(final ChatType type, final Player player, final String target, final String text) {
        ...
        final int levelRequired = chatSettings.tradeChatLevel();
        if (player.getLevel() < levelRequired) {
            player.sendPacket(new ServerPacket[] { (ServerPacket)SystemMessage.getSystemMessage(SystemMessageId.TRADE_CHAT_CANNOT_BE_USED_BY_USERS_LV_S1_OR_LOWER).addInt(levelRequired) });
            return;
        }
    ...
    public ChatType[] getChatTypeList() {
        return ChatTrade.CHAT_TYPES;
    }
    
    static {
        CHAT_TYPES = new ChatType[] { ChatType.TRADE };
    }
}
